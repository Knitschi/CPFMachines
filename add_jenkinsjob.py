...
from . import setup_docker_container_with_cpf_jobs
...
def add_cpf_job(job_base_name, jenkins_directory, build_repository_address, jenkinsjob_repository_address ):
    ...
    job_name = setup_docker_container_with_cpf_jobs.get_job_name(job_base_name)
    job_dir = _get_job_dir(jenkins_directory, job_name)
    _clear_directory(job_dir)

    # add a new config.xml file
    setup_docker_container_with_cpf_jobs.configure_job_config_file(job_dir, job_name, build_repository_address, jenkinsjob_repository_address)
